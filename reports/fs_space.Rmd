...
evar_plot <- ggplot(data = fs_bank_mets, aes(x = rank, y = sabund, color = evar, group = sim)) +
  ...
  geom_line(data = filter(fs_bank_mets, evar <= quantile(fs_bank_mets$evar, probs = .001)[1]), alpha = .5, size = .5) +
  scale_colour_viridis_c(option = "plasma", end = .8) + 
  theme_bw() +
  theme(legend.position = "top")
...
skew_violin <- ggplot(data = skew_distance, aes(x = 0, y = skew)) +
  geom_violin() +
  geom_point(aes(x = 0, y = conditionalsads::rad_skew(portal_sad$sabund)))
skew_violin

gridExtra::grid.arrange(grobs = list(skew_violin, centroid_dist_plot))
```
