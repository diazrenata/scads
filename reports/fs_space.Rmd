...
fs_leg_coeffs <- apply(as.matrix(1:ncol(fs_bank)), MARGIN = 1, 
                       FUN = function(sim_index, fs_bank_M) 
                         return(legendre_approx(filter(fs_bank_M, sim == sim_index)$sabund, nleg = 10)$coefficients), fs_bank_M = fs_bank_M)
...
portal_leg_coeffs <-data.frame(coeff_value = legendre_approx(portal_sad$sabund, nleg = 10)$coefficients) %>%
  mutate(coeff_name = names(legendre_approx(portal_sad$sabund, nleg = 10)$coefficients),
...
estimates_df <- data.frame(
  rank = c(1:nspp),
  centroid = legendre_generate(coeff_values = portal_leg_coeffs$centroid_value, nleg = 10, nspp = nspp),
  closest_fs = legendre_generate(coeff_values = filter(fs_leg_coeffs_M, sim == filter(fs_dist, sim_rank == 1)$sim)$coeff_value, nleg = 10, nspp = nspp),
  farthest_fs = legendre_generate(coeff_values = filter(fs_leg_coeffs_M, sim == filter(fs_dist, sim_rank == max(fs_dist$sim_rank))$sim)$coeff_value, nleg = 10, nspp = nspp),
  portal = legendre_generate(coeff_values = portal_leg_coeffs$coeff_value, nleg = 10, nspp = nspp)) %>%
