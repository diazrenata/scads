...
if(load_samples) {
  load(here::here("reports", "centroids_intuition", "sensitivity_stash_plants.Rds"))
} else {
set.seed(1977)
...
if(save_samples) {
  save(sim_sads, file = here::here("reports", "centroids_intuition", "sensitivity_stash_plants.Rds"))
}
}
...
sim_coeff_plot <- ggplot(data = sim_estimates, aes(x = as.factor(varindex), y = coefficient)) + 
 # geom_point(alpha = .05) +
  geom_boxplot() +
