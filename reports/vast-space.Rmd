...
min_skew_plots <- ggplot(data = filter(di_df_M, variable %in% c("skew_max", "skew_min")), aes(x =S, y = N, color = val)) +
    geom_point(alpha = 0) +
  geom_point(data = filter(di_df_M, variable == "skew_min")) +
  theme_bw() +
  scale_color_viridis_c(end = .8, option = "magma") +
  ggtitle("Minimum skewness")

min_skew_plots


max_skew_plots <- ggplot(data = filter(di_df_M, variable %in% c("skew_max", "skew_min")), aes(x =S, y = N, color = val)) +
  geom_point(alpha = 0) +
  geom_point(data = filter(di_df_M, variable == "skew_max")) +
  theme_bw() +
  scale_color_viridis_c(end = .8, option = "magma") +
  ggtitle("Maximum skewness")
