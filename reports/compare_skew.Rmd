...
fs_bank_skews <- fs_bank_M %>%
  ...
  summarize(skew_1 = e1071::skewness(abund, type = 1),
            ...
            evar = conditionalsads::evar(abund)) %>%
  ungroup()# %>%
...
```{r add evar, fig.width=12}

evar_skew_plot <- ggplot(data = fs_bank_skews, aes(x = skew_1, y = evar)) +
  geom_point(alpha = .7) +
  ggtitle("Evar v Skew 1")


r_evar_skew_plot <- ggplot(data = fs_bank_skews, aes(x = skew_1_rank, y = evar_rank)) +
  geom_point(alpha = .7) +
  ggtitle("Evar v Skew 1 Rank")

gridExtra::grid.arrange(grobs = list(evar_skew_plot, r_evar_skew_plot), nrow = 1)

all((fs_bank_skews$skew_1_rank == fs_bank_skews$evar_rank))
